...
ler_taxas :- %recebe as taxas e as define em predicados dinâmicos
    ...
    retractall(dias_infec_max(_)), assertz(dias_infec_max(T)).
...
vacinar_random(N) :-
    ...
    random_between(1, N, X),
    atomic_list_concat([p,X], Pessoa), %Concatena o átomo como nome da pessoa 'pN'
    estado(Pessoa, suscetivel), %Precisa ser sucetivel
    vacinar(Pessoa),
    !.
...
vacinar(Pessoa):-
    retractall(estado(Pessoa, _)),
    assertz(estado(Pessoa, vacinado)).
...
mostrar_estados :-
    ...
    findall(Pessoa, estado(Pessoa, vacinado), ListaVac),
